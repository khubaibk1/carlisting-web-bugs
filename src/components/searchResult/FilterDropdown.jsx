...
const FilterDropdown = ({ options, initialValue, tab, subTab ,HoverPurple}) => {
    ...
    if (tab === 'used' && (subTab === 'clear' || subTab === 'all')) {
        ...
        return (
            ...
            {isOpen && (
                <div className={`absolute mt-1  z-[100] w-[323px] transform translate-x-[-2px] h-[300px] ${scroll?overflow-y-scroll:''} bg-white shadow-lg rounded-md`} >
                    {options.map((option) => (
                        <div
                            key={option}
